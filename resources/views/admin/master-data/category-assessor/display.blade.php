<table class="table datatables table-bordered table-hover table-data" id="table-category-assessor">
    <thead>
        <tr class="text-uppercase">
            <th class="w-5p">No</th>
            <th class="w-15p">Bidang</th>
            <th>Nama</th>
            <th>Deskripsi</th>
    ...
    <tbody>
        @forelse ($data as $key => $item)
            <tr>
                ...
                <td class="w-15p">{{ $item->field }}</td>
                <td>{{ $item->name }}</td>
                <td>{{ $item->description }}</td>
                <td class="w-5p">
                    <div class="btn-group btn-group-sm" role="group">
                        <button type="button" class="btn p5x btn-sm btn-outline-warning"
                            onclick="openModal('category-assessor','edit', {{ $item->id }})"><span
